    ...
    boost::asio::async_read(socket_,
                            ...
                            [this](boost::system::error_code ec, std::size_t /*length*/)
                            {
                                if (!ec)
                                {
                                    ...
                                    std::string message(received_message);
                                    delete[] received_message;

                                    if (message.rfind("SYSTEM:", 0) == 0)
                                    {
                                        ui_->textBrowser->append("<i>" + QString::fromStdString(message.substr(7)) + "</i>");
                                    }
                                    else
                                    {
                                        // Обычное сообщение
                                        ui_->textBrowser->append(QString::fromStdString(message));
                                    }
