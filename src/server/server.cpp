...
void chat_room::join(chat_participant_ptr participant, std::string username)
{
    ...
    chat_message welcome_message;
    std::string notification = "SYSTEM: User [" + username + "] has joined the chat.";

    size_t notification_size = notification.size();
    welcome_message.body_length(notification_size);
    std::memset(welcome_message.body(), '\0', notification_size + 1);
    std::memcpy(welcome_message.body(), notification.c_str(), notification_size);

    welcome_message.encode_header();

    for (auto p : participants_)
        p->deliver(welcome_message);
...
void chat_room::deliver(chat_message& msg, chat_participant_ptr participant)
{
    ...
    char time_buffer[6];
    std::strftime(time_buffer, sizeof(time_buffer), "%H:%M", std::localtime(&now));

    chat_message transmitted_message;
    std::string full_message = "[" + std::string(time_buffer) + "] " + username + msg.body();

    size_t transmitted_message_size = full_message.size();
    transmitted_message.body_length(transmitted_message_size);
    std::memset(transmitted_message.body(), '\0', transmitted_message_size + 1);
    std::memcpy(transmitted_message.body(), full_message.c_str(), transmitted_message_size);
